...
function initValidation() {
    ...
    var hiddenText = jQuery('#hidden-msg');
    var thankyouText = jQuery('#thankyou-msg');
    var popup = jQuery('#popup1');
    var popup_activator = jQuery('a#popup_1');
    var formBox = jQuery('.flip-mask');

    jQuery('form.validate-form').each(function(){
        var form = jQuery(this).attr('novalidate', 'novalidate');
        var successFlag = true;
        var inputs = form.find('input, textarea, select');
        var flag = form.data('thank-page');
        ...
        function validateForm(e) {
            ...
            if(form.hasClass('not-send-ajax')){
                ...
                if (successFlag) {
                    jQuery.ajax({
                        ...
                        success: function (data) {
                            console.log(data);
                            popup.find('p').text(data.message);
                            popup_activator.click();
                            if (form.hasClass('invitation-form')) {
                                window.location.replace('/login');
                            }
                        },
                        error: function (data) {
                            popup.find('p').text(JSON.parse(data.responseText).error);
                            popup_activator.click();
                        }
                    });
                }
